...
$out = if ($OutputName) { $OutputName } else { "ecom_$timestamp.zip" }
$outPath = Join-Path $scriptRoot $out
$tmp = Join-Path $env:TEMP "ecom_zip_$timestamp"
...
Copy-WithoutNodeModules (Join-Path $scriptRoot 'ecom') (Join-Path $tmp 'ecom')
